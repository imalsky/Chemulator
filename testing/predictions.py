#!/usr/bin/env python3
"""
Minimal Stable LPV Koopman AE (.pt2) prediction + plotting.

- Loads exported program (torch.export) and runs batched K queries by using K as the batch size.
- Handles model outputs shaped [K,1,S], [1,K,S], or [K,S].
"""
...
SAMPLE_IDX = 0
Q_COUNT = 100
XMIN, XMAX = 1e-3, 1e8  # time-axis (s) for plotting
...
def _load_meta_json(pt2_path: Path) -> dict:
    mp = pt2_path.with_suffix(pt2_path.suffix + ".meta.json")
    if mp.exists():
...
def main():
    ...
    gm = ep.module()    # GraphModule; no need to call .eval()
    ...
    if out.dim() == 3:
        if out.size(1) == 1:
            ...
            # Unexpected 2-step shape; flatten and realign K-dependent arrays.
    ...
    for i in range(n_cols):
        ...
        c = colors[i]
        ax.loglog(t_gt_plot, y_gt_plot[:, i], '-', lw=1.8, alpha=0.9, color=c, label=name if i == 0 else None)
        if t_gt_plot.size:
