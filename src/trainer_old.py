#!/usr/bin/env python3
"""
Trainer for Stable LPV Koopman Autoencoder (no‑FiLM path)
=========================================================

Kept features:
- Forward loss across all Δt offsets.
- Unified rollout loss: vectorized when TF=0; loop when TF>0.
- Teacher‑forcing schedule (none | linear).
- Rollout horizon warmup.
- Cosine LR with linear warmup; AdamW; manual grad clip (fused‑safe).
- Lightning checkpointing + best model export.
- Adaptive stiff loss (per‑decade log10 + z‑space MSE) with mask + time weights.
- Optional KL term (VAE mode) via model.kl_loss.
- Optional semigroup loss.
- Epoch summary line.

Removed: explicit FiLM handling in trainer. Models may still condition internally.
Works with both uploaded models by calling rollout_vectorized with positional args
for dt_steps_norm (avoids keyword mismatch).

Batch = (y_i, dt_norm_offsets, y_j, g, [aux], [mask])
  y_i: [B,S]
  dt:  [B,K] (absolute normalized OFFSETS)
  y_j: [B,K,S]
  g:   [B,G]
  mask:[B,K] or None
"""
...
MAX_RESIDUAL = 1e4  # cap residuals in MSE

# ------------------------------ Adaptive Stiff Loss ------------------------------

class AdaptiveStiffLoss(nn.Module):
    """Per‑decade MAE in log10(x) + z‑space MSE, with species + time weights.

    manifest[per_key_stats][key] with log_min/log_max or log_mean/log_std.
    Supported methods: "log-standard", "log-min-max".
    """
    def __init__(
        self,
        manifest: dict,
        ...
        epsilon_phys: float = 1e-25,  # kept in signature for compat
        ...
        if not isinstance(manifest, dict):
            raise TypeError("[AdaptiveStiffLoss] manifest must be dict")
        if "per_key_stats" not in manifest or "normalization_methods" not in manifest:
            raise KeyError("[AdaptiveStiffLoss] need 'per_key_stats' and 'normalization_methods'")
        if not species_keys:
            raise RuntimeError("[AdaptiveStiffLoss] species_keys empty")
        ...
        for key in self.species_keys:
            if key not in per_key_stats or key not in norm_methods:
                raise KeyError(f"[AdaptiveStiffLoss] missing stats/method for '{key}'")
            stats = per_key_stats[key]
            method = str(norm_methods[key]).lower()

            if method == "log-standard":
                if "log_mean" not in stats or "log_std" not in stats:
                    raise KeyError(f"[AdaptiveStiffLoss] '{key}' needs log_mean/log_std")
                ...
                if not (math.isfinite(log_mean) and math.isfinite(log_std) and log_std > 0):
                    raise ValueError(f"[AdaptiveStiffLoss] bad log_mean/log_std for '{key}'")
                log_bias = log_mean
                log_scale = log_std
                ...
                if "log_min" not in stats or "log_max" not in stats:
                    raise KeyError(f"[AdaptiveStiffLoss] '{key}' needs log_min/log_max")
                lo = float(stats["log_min"])
                hi = float(stats["log_max"])
                if not (math.isfinite(lo) and math.isfinite(hi) and hi > lo):
                    raise ValueError(f"[AdaptiveStiffLoss] bad log_min/log_max for '{key}'")
                log_bias = lo
                log_scale = hi - lo
            else:
                raise RuntimeError(f"[AdaptiveStiffLoss] unsupported method '{method}'")

            if not (math.isfinite(log_bias) and math.isfinite(log_scale) and log_scale > 0):
                raise ValueError(f"[AdaptiveStiffLoss] non‑finite bias/scale for '{key}'")
        ...
        for name, buf in [
            ...
            if not torch.isfinite(buf).all():
                raise RuntimeError(f"[AdaptiveStiffLoss] non‑finite buffer '{name}'")
    ...
    def forward(
        ...
        if pz.shape[-1] != tz.shape[-1]:
            raise ValueError("[AdaptiveStiffLoss] last‑dim mismatch")

        mse_per_sample = (pz - tz).square().mean(dim=-1)

        # Fast analytic z -> log10(x)
        if self._fast_ok and self._log_scale_vec.numel() == pz.shape[-1]:
            pred_log10 = pz * self._log_scale_vec + self._log_bias_vec
            true_log10 = tz * self._log_scale_vec + self._log_bias_vec
        else:
        ...
        phys_err_species = torch.nan_to_num((pred_log10 - true_log10).abs(), nan=0.0, posinf=1e6, neginf=0.0)
        if self.w_species.numel() == phys_err_species.shape[-1]:
            phys_err_species = phys_err_species * self.w_species
        phys_per_sample = phys_err_species.mean(dim=-1)
        ...
        if t_norm is not None:
            ...
            while t.ndim < phys_per_sample.ndim:
                t = t.unsqueeze(-1)
            t01 = torch.clamp(t, 0.0, 1.0).to(dtype=phys_per_sample.dtype, device=phys_per_sample.device)
        ...
        if return_components:
            return {"total": total, "mse": mse_scalar.detach(), "frac": phys_scalar.detach()}
        return total
...
def _validate_batch(batch, batch_idx: int = 0) -> Tuple[int, int, int]:
    if len(batch) < 4:
        raise RuntimeError(f"Batch has {len(batch)} items; need >=4 (y_i, dt, y_j, g, [aux], [mask])")
    y_i, dt, y_j, g = batch[:4]
    if not all(isinstance(x, torch.Tensor) for x in (y_i, dt, y_j, g)):
        raise RuntimeError("Batch must contain tensors for y_i, dt, y_j, g")
    if y_i.ndim != 2: raise RuntimeError(f"y_i must be [B,S], got {tuple(y_i.shape)}")
    if dt.ndim != 2:  raise RuntimeError(f"dt_norm must be [B,K], got {tuple(dt.shape)}")
    if y_j.ndim != 3: raise RuntimeError(f"y_j must be [B,K,S], got {tuple(y_j.shape)}")
    if g.ndim != 2:   raise RuntimeError(f"g must be [B,G], got {tuple(g.shape)}")
    ...
    Bg, _ = g.shape
    if not (B == Bdt == By == Bg):
        raise RuntimeError(f"Batch size mismatch: y_i={B}, dt={Bdt}, y_j={By}, g={Bg}")
    if K != Ky:
        raise RuntimeError(f"K mismatch: dt={K}, y_j={Ky}")
    if S != Sy:
        raise RuntimeError(f"S mismatch: y_i={S}, y_j={Sy}")
...
def _masked_mse(
    ...
    pred32 = pred.float(); target32 = target.float()
    resid = pred32 - target32
    if not torch.isfinite(resid).all():
        print("[WARN] _masked_mse: non‑finite residuals; replacing with zeros.")
    ...
    if mask is not None:
        ...
        loss = (per_bk * m).sum() / m.sum().clamp_min(1.0)
    else:
        loss = per_bk.mean()
...
class EpochSetterCallback(Callback):
    def on_train_epoch_start(self, trainer: pl.Trainer, pl_module: "AutoEncoderModule") -> None:
        ...
        pl_module.log("tf_prob", float(pl_module._get_teacher_forcing_prob(epoch)), on_epoch=True)
        pl_module.log("rollout_horizon", float(pl_module._get_rollout_horizon(epoch)), on_epoch=True)

class EpochSummaryLineCallback(Callback):
    """One‑line epoch summary with timing."""
    def __init__(self):
        super().__init__()
        self._header_printed = False
        self._printed_epoch = -1
        self._t0 = None
        self._w_num, self._w_tf, self._w_H, self._w_time = 10, 5, 3, 8
    ...
    def _maybe_print_header(self):
        if self._header_printed: return
        hdr = (
            "EPOCH | "
            f"{'train':>{self._w_num}} | "
            f"{'val':>{self._w_num}} | "
    ...
    def _emit(self, trainer: pl.Trainer, pl_module: "AutoEncoderModule", elapsed_s: float) -> None:
        ...
        def _getf(key: str) -> float:
            if key not in m: return float("nan")
            v = m[key]
            if torch.is_tensor(v): v = v.detach().float().cpu().item()
            return float(v)

        train_v = _getf("train_total")
        if math.isnan(train_v): train_v = _getf("train_mse")
        if math.isnan(train_v): train_v = _getf("train")
        if math.isnan(train_v): train_v = float(getattr(pl_module, "_last_train_metric", float("nan")))
        val_v = _getf("val")
        ...
        with torch.no_grad():
            ...
            for p in pl_module.model.parameters():
                ...
                if torch.isfinite(ps): p2 += float(ps.item())
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, pl_module: "AutoEncoderModule") -> None:
        if self._printed_epoch == trainer.current_epoch: return
        elapsed = (time.perf_counter() - self._t0) if self._t0 is not None else float("nan")
        self._emit(trainer, pl_module, elapsed)
        self._printed_epoch = trainer.current_epoch
...
class AutoEncoderModule(LightningModule):
    ...
    def _setup_adaptive_stiff_loss(self) -> None:
        ...
        if tcfg.get("loss_mode", "adaptive_stiff") != "adaptive_stiff":
            self.criterion = None
        ...
        try:
            ...
            raise RuntimeError("cfg['paths']['processed_data_dir'] missing/invalid") from e

        norm_path = processed_dir / "normalization.json"
        if not norm_path.exists():
            raise FileNotFoundError(
                f"normalization.json not found at: {norm_path}. Run preprocessing or fix cfg.paths.processed_data_dir."
            )
        ...
        if not has_dt:
            msg = "normalization.json missing 'dt' block (log_min, log_max)."
            if need_dt:
                raise ValueError(msg)
            else:
                print(f"[WARN] {msg} Proceeding: require_dt_stats=False.")

        data_cfg = self.cfg.get("data", {})
        species_keys = (
            data_cfg.get("target_species")
            or manifest.get("meta", {}).get("target_species")
            or manifest.get("meta", {}).get("species_variables")
        )
        if not species_keys:
            raise ValueError("Could not determine species_keys (cfg.data.target_species or manifest.meta.*)")
        species_keys = list(species_keys)

        pks = set((manifest.get("per_key_stats") or {}).keys())
        missing = [k for k in species_keys if k not in pks]
        if missing:
            raise RuntimeError("Species missing from manifest.per_key_stats: " + ", ".join(missing))
        ...
        print(
            ...
            f"rel_cap={rel_cap}, time_edge_gain={time_edge_gain}, dt_present={has_dt}"
        )

    def configure_optimizers(self):
        try:
            opt = AdamW(self.model.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay, fused=True)
        except TypeError:
            opt = AdamW(self.model.parameters(), lr=self.learning_rate, weight_decay=self.weight_decay)

        warm = LinearLR(opt, start_factor=0.1, end_factor=1.0, total_iters=max(1, self.warmup_epochs))
    ...
    def _get_teacher_forcing_prob(self, epoch: int) -> float:
        if self.tf_mode == "none": return 0.0
        if self.tf_mode == "linear":
            if epoch >= self.tf_end_epoch: return self.tf_end_p
    ...
    def _get_rollout_horizon(self, epoch: int) -> int:
        if self.max_rollout_horizon <= 1: return 1
    ...
    def _compute_rollout_loss_unified(
        ...
        """Vectorized rollout when TF=0; loop w/ TF when TF>0."""
        ...
        if tf_prob < 1e-8:
            ...
            if self.use_adaptive_stiff and self.criterion is not None:
                return self.criterion(pred_norm=y_pred, true_norm=y_tgt, t_norm=dt_offsets[:, :H], mask=rmask)
        ...
        for k in range(H):
            # Encode/step/decode (no FiLM here; if model conditions internally, it will handle it)
            if hasattr(self.model, "encode") and hasattr(self.model, "decode"):
                z = self.model.encode(y_curr, g)
            ...
            if use_delta:
                ...
                y_next = self.model._head_from_logprobs(log_p)
            else:
                y_next = self.model._softmax_head_from_logits(logits)

            preds.append(y_next)

            # TF mix for next step
            if tf_prob >= 1.0:
                y_curr = y_j[:, k, :]
            else:
                if self.training:
                    coin = (torch.rand(B, device=y_i.device) < tf_prob).unsqueeze(-1)
    ...
    def _compute_semigroup_loss(self, y_i: torch.Tensor, g: torch.Tensor) -> Optional[torch.Tensor]:
        ...
        B = y_i.shape[0]
        ln10 = math.log(10.0)
        dt_tot_norm = torch.rand(B, device=y_i.device, dtype=torch.float32)
        dt_tot_phys = torch.exp((log_min + dt_tot_norm * rng) * ln10)
        u = torch.rand(B, device=y_i.device, dtype=torch.float32)
        ...
        dt_a_norm = dt_a_norm.clamp_(0.0, 1.0).to(y_i.dtype)
        dt_b_norm = dt_b_norm.clamp_(0.0, 1.0).to(y_i.dtype)
        dt_tot_norm = dt_tot_norm.clamp_(0.0, 1.0).to(y_i.dtype)
    ...
    def on_before_optimizer_step(self, optimizer) -> None:
        if self.grad_clip and float(self.grad_clip) > 0.0:
            try:
        ...
        for p in self.model.parameters():
            if p is not None and p.grad is not None:
                ...
                if torch.isfinite(ps): g2 += float(ps.item())
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        if self.use_adaptive_stiff and self.criterion is not None:
            comp = self.criterion(pred_norm=pred, true_norm=y_j, t_norm=dt_offsets, mask=mask, return_components=True)
        ...
        # Drop caches to free graph
        if hasattr(self.model, "_cached_rollout_z0"): self.model._cached_rollout_z0 = None
        if hasattr(self.model, "_cached_rollout_film"): self.model._cached_rollout_film = None
        return total
...
class Trainer:
    """Construct Lightning objects and run training."""
    def __init__(
        self,
        model: nn.Module,
    ...
    def train(self) -> float:
        ...
        if bool(compile_cfg.get("enable", False)):
            ...
            try:
                self.model = torch.compile(self.model, backend=backend, mode=mode, dynamic=dynamic, fullgraph=fullgraph)
                try:
                    self.logger.info(
                        f"torch.compile enabled: backend={backend}, mode={mode}, dynamic={dynamic}, fullgraph={fullgraph}"
                    )
                except Exception:
                    print(
                        f"[INFO] torch.compile enabled: backend={backend}, mode={mode}, dynamic={dynamic}, fullgraph={fullgraph}"
                    )
            except Exception as e:
                try:
        ...
        monitor_metric = self.optuna_monitor  # usually "val"
        ...
        es_cb = EarlyStopping(monitor=monitor_metric, mode="min", patience=es_patience) if es_patience > 0 else None

        callbacks = [ckpt_cb, lr_cb, EpochSetterCallback(), EpochSummaryLineCallback()]
            ...
            if _EMA_CB is not None and isinstance(_EMA_CB, type) and issubclass(_EMA_CB, Callback):
                ...
                try:
                    ...
                    try:
                        ...
                        print("[WARN] EMA callback could not be constructed; skipping.")
            else:
                print("[WARN] EMA callback not compatible; skipping.")
        ...
        if bool(swa_cfg.get("enable", False)):
            if _SWA_CB is not None and isinstance(_SWA_CB, type) and issubclass(_SWA_CB, Callback):
                ...
                print("[WARN] SWA callback not compatible; skipping.")
        ...
        if self.optuna_trial is not None:
            try:
                ...
                if isinstance(_PruneCB, type) and issubclass(_PruneCB, Callback):
                    try:
                        callbacks.append(_PruneCB(self.optuna_trial, monitor=monitor_metric))
                    except Exception as e:
                        try:
                    ...
                    print("[WARN] Optuna pruning callback incompatible; skipping.")
            except Exception as e:
                try:
        ...
        if es_cb is not None: callbacks.append(es_cb)
        ...
        trainer.fit(module, train_dataloaders=self.train_loader, val_dataloaders=self.val_loader, ckpt_path=resume_ckpt)
        ...
        if ckpt_cb.best_model_path:
            ...
            torch.save({"model": model_state, "config": self.cfg}, self.work_dir / "best_model.pt")
            msg = (
                f"Exported best_model.pt (metric {monitor_metric}={best_val:.4e})" if best_val is not None else "Saved best_model.pt (no best_val)"
            )
            try:
        ...
        # Model soup across top‑k checkpoints
        ms_cfg = tcfg.get("model_soup", {})
        if bool(ms_cfg.get("enable", False)) and int(ms_cfg.get("top_k", 1)) > 1:
            try:
                best_k = ckpt_cb.best_k_models  # path -> score
                    ...
                    for pth in ckpt_paths:
                        ...
                        if not avg_state:
                            for k, v in model_sd.items():
                                if torch.is_tensor(v): avg_state[k] = v.detach().clone().float()
                            n = 1
                        else:
                            for k, v in model_sd.items():
                            ...
                            n += 1
                    if n > 0:
                        for k in list(avg_state.keys()): avg_state[k] /= float(n)
                        torch.save({"model": avg_state, "soup_members": ckpt_paths, "config": self.cfg}, self.work_dir / "soup_model.pt")
