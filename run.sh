...
if [ "${SKIP:-0}" -eq 0 ]; then
  if command -v srun >/dev/null 2>&1; then
    OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 srun -n 32 "$PY" -c "from pathlib import Path; from utils import load_json_config, setup_logging; from preprocessor import DataPreprocessor; cfg = load_json_config(Path('config/config.jsonc')); setup_logging(); DataPreprocessor(cfg).run()"
  elif command -v mpiexec >/dev/null 2>&1; then
    OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 mpiexec -n 32 "$PY" -c "from pathlib import Path; from utils import load_json_config, setup_logging; from preprocessor import DataPreprocessor; cfg = load_json_config(Path('config/config.jsonc')); setup_logging(); DataPreprocessor(cfg).run()"
  else
